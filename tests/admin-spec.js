...
describe('Angular Assignment Test Suite - Admin ', function () {
  ...
  it('should redirect to home page if successful admin login',function()
  {
    ...
    browser.sleep(1000); // need buffer to load data - force first user to be admin since we using in-memory api for users
    browser.get('http://localhost:4200/users');

    browser.sleep(1000);
    element.all(by.xpath(".//a[contains(text(),'Edit')]")).last().click();
